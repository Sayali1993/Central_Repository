...
list_5th_dic = []
for key1, val1 in input_dic.items():
    if type(val1) == dict:
        for key2, val2 in val1.items():
            if type(val2) == dict:
                for key3, val3 in val2.items():
                    if type(val3) == dict:
                        for k4, val4 in val3.items():
                            if type(val4) == dict:
                               list_5th_dic.append(val4)

var1 = {k: v for k, v in flatten_dict(list_5th_dic[0])}
list_item_1 = []
[list_item_1.append(k + ' : ' + v) for k, v in var1.items()]
input_dic['dependencies']['@types/express']['requires']['@types/server/nodes'] = list_item_1

var2 = {k: v for k, v in flatten_dict(list_5th_dic[1])}
list_item_2 = []
